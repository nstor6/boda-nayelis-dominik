...
const content = {
  ...
  gifts: () => `${heading('🎁','Mesa de regalos')}<p>El mejor regalo es que estés presente en este día tan especial, pero si deseas hacernos un detalle lo puedes hacer aquí:</p>${config.iban ? `<p class="iban">${escapeHtml(config.iban)}</p><button class="action" id="copy-iban">Copiar número de cuenta</button><p role="status" id="copy-status"></p>` : '<p class="notice">Pronto compartiremos el número de cuenta. Si lo necesitas, puedes contactar con nosotros.</p>'}`,
  contact: () => `${heading('✉','Hablemos')}<p>Pronto compartiremos los datos de contacto.</p>`,
  rsvp: () => `${heading('♡','¿Nos acompañas?')}<p>Tu presencia es importante para nosotros. Confirma tu asistencia <strong>antes del 7 de mayo de 2027</strong>.</p><p class="small">Indica también los menús especiales o infantiles, el autobús o si no puedes asistir.</p>${config.formUrl && /^https:\/\//i.test(config.formUrl) ? externalLink(config.formUrl,'Abrir formulario de asistencia') : !config.bridePhone || !config.groomPhone ? '<p class="notice">La confirmación de asistencia estará disponible próximamente.</p>' : `<form id="rsvp-form"><label>Nombre y apellidos<input name="name" autocomplete="name" maxlength="120" required></label><label>¿Asistirás?<select name="attendance"><option value="Sí">Sí, ¡allí estaré!</option><option value="No">No podré asistir</option></select></label><div id="guest-details"><label>Nombres de acompañantes<textarea name="companions" maxlength="400" placeholder="Si vienes acompañado/a"></textarea></label><div class="form-grid"><label>Adultos (incluyéndote)<input name="adults" type="number" min="1" max="30" value="1" required></label><label>Menús infantiles<input name="children" type="number" min="0" max="30" value="0" required></label></div><label>Menús especiales o alergias<textarea name="menus" maxlength="600" placeholder="Indica el nombre y lo que necesita cada persona"></textarea></label><label>¿Necesitas autobús?<select name="bus"><option>No</option><option>Sí</option><option>Pendiente de confirmar</option></select></label></div><label>¿A quién quieres enviárselo?<select name="recipient"><option value="bride">Nayelis</option><option value="groom">Dominik</option></select></label><label>Un mensaje para nosotros<textarea name="message" maxlength="600"></textarea></label><p class="notice">Prepararemos un mensaje con tus respuestas. Deberás enviarlo en WhatsApp para confirmar tu asistencia. No se guardan datos en esta web.</p><button class="action" type="submit">Preparar mi confirmación</button><div id="form-status" role="status"></div></form>`}`
};
